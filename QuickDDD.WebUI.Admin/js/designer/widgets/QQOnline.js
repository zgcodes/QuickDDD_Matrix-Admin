...
define('widgets/QQOnline', function (require, exports, module) {
    ...
    WX_Widget.QQOnline = {
        ...
        showWidgetTemplate: function () {
            return [
			'<section class="wQQOnline ${FWidgetId}">',
            '<style>',
            'section.wQQOnline.${FWidgetId} {',
            '  padding: ${FStyle.Margin|padding};',
            '}',
            'section.wQQOnline.${FWidgetId} a {',
            ...
            '}',
            'section.wQQOnline.${FWidgetId} .divicon {',
            '  font-size: ${FStyle.IconSize};',
            '  color: ${FStyle.IconColor};',
            '}',
            'section.wQQOnline.${FWidgetId} .divicon > img {',
            ...
            '}',
            'section.wQQOnline.${FWidgetId} .divtext {',
            ...
            '$${FStyle.CssHtml}',
            '</style>',
            '<a target="_blank" {@if (FData.QQ)}href="http://wpa.qq.com/msgrd?v=3&uin=${FData.QQ}&site=qq&menu=yes"{@/if}>',
        ...
        editWidgetTemplate: [
            ...
            '    <label>显示标题：<span class="asterisk">*</span></label><span class="help-inline">（限20位）</span>',
            '    <div class="controls">',
            '        <input class="form-control" type="text" maxlength="20" data-tmpldata="Title" value="$${FData.Title}" placeholder="请输入显示标题">',
            '    </div>',
            '</div>',
            '<div class="form-group">',
            '    <label>QQ号码<span class="asterisk">*</span></label><span class="help-inline"></span>',
            '    <div class="controls">',
            '        <input class="form-control" type="text" maxlength="20" data-tmpldata="QQ" value="$${FData.QQ}" placeholder="请输入QQ号码">',
            '    </div>',
            '</div>',
            //'<div class="form-group">',
        ...
        styleWidgetTemplate: [
            ...
            '	        <textarea id="txt_html" class="input-xxlarge" rows="15">{@if (FStyle.CssHtml=="")}/*section.wQQOnline.${FWidgetId} { }*/{@else}$${FStyle.CssHtml}{@/if}</textarea>',
            '	    </div>',
            '	</div>',
            '</div>'].join("\n"),
        ...
        initWidgetEditNode: function ($showNode, $editNode, data) {
            ...
            var limitObjs = [];
            $.each(WX_Dom.getEditNode().find('input[data-tmpldata="QQ"]'), function (i, ele) {
                limitObj = {};
        ...
        returnWidgetEditData: function ($editNode) {
            ...
            var inputData = WX_Utils.modelData($editNode, 'Title QQ Icon', false); // 通用格式获取数据
            var data = {
                //FWidgetCode: 'Button',
    ...
    module.exports = WX_Widget.QQOnline;
});
