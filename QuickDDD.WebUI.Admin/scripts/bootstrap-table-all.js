...
!function ($) {
    ...
    var calculateObjectValue = function (self, name, args, defaultValue) {
        ...
        if (typeof name === 'string') {
        	...
        	// support obj.func1.func2
        	var getObject = function (parentObject, name){
            ...
            //增加参数注入 update bu yujinjin
            if(/\(.*\)/.test(name)){
            	var execNames = /(.+)\((.+)\)/.exec(name);
            	func = getObject(window, execNames[1]);
            	var customParameters = execNames[2].split(",");
            	for(var i=0;i<customParameters.length;i++){
            		customParameters[i] = $.trim(customParameters[i]);
            		if(/^'.*'|".*"$/.test(customParameters[i])){
            			customParameters[i] = customParameters[i].substring(1, customParameters[i].length-1);
            		} else {
            			customParameters[i] = getObject(window,customParameters[i]);
            		}
            	}
            	args = args.concat(customParameters);
            } else {
            	func = getObject(window, name);
            }
...
//            }
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        //update by yujinjin 增加formatShowingRows函数的增加（pageSize）页面记录数参数
        html.push(
            '<div class="pull-' + this.options.paginationDetailHAlign + ' pagination-detail">',
            '<span class="pagination-info">',
            this.options.onlyInfoPagination ? this.options.formatDetailPagination(this.options.totalRows) :
            this.options.formatShowingRows(this.pageFrom, this.pageTo, this.options.totalRows, this.options.pageSize),
            '</span>');
...
!function ($) {
    ...
    BootstrapTable.prototype.init = function () {
        ...
        if (!this.options.reorderableRows) {
        	//update add code by yujinjin reorderableRows参数为false就不显示了，不做任何初始化了
        	_init.apply(this, Array.prototype.slice.apply(arguments));
            return;
        }
